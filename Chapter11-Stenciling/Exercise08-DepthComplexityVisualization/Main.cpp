//***************************************************************************************
// Chapter 11 Exercise 08: Use the stencil buffer to visualize depth complexity.
//  Depth Complexity 0 = White
//  Depth Complexity 1 = Green
//  Depth Complexity 2 = Blue
//  Depth Complexity 3 = Yellow
//  Depth Complexity 4 = Red
// 
...
class BlendApp final : public D3DApp
{
public:
    ...
    int showCmd)
{
    ...
    {
        MessageBoxA(nullptr, e.what(), "C++ Exception", MB_OK);
        return 2;
    }
}
...
void BlendApp::Draw(const GameTimer& gt)
{
    ...
    mCommandList->ClearRenderTargetView(
        backBuffView, (float*)&mMainPassConstants.FogColor, 0, nullptr);
    ...
    mCommandList->SetPipelineState(mPSOs["vizPSO"].Get());
    constexpr UINT kMaxDepthComplexity = 5;
    for (UINT i = 0; i < kMaxDepthComplexity; ++i)
    {
...
void BlendApp::UpdateMaterialCBs(const GameTimer& gt)
{
    ...
    for (auto& [_, mat] : mMaterials)
    {
        ...
        {
            matTransform = XMLoadFloat4x4(&mat->MatTransform);
...
void BlendApp::UpdateMainPassCB(const GameTimer& gt)
{
    ...
    mMainPassConstants.Lights[0].Strength = { 0.9f, 0.9f, 0.8f };
    mMainPassConstants.Lights[1].Direction = { -0.57735f, -0.57735f, 0.57735f };
    mMainPassConstants.Lights[1].Strength = { 0.3f, 0.3f, 0.3f };
    mMainPassConstants.Lights[2].Direction = { 0.0f, -0.707f, -0.707f };
    mMainPassConstants.Lights[2].Strength = { 0.15f, 0.15f, 0.15f };
...
void BlendApp::BuildRootSignature()
{
    ...
    slotRootParameter[1].InitAsConstantBufferView(0); // cbPerObject
    slotRootParameter[2].InitAsConstantBufferView(1); // cbPerPass
    slotRootParameter[3].InitAsConstantBufferView(2); // cbMaterial
...
void BlendApp::BuildShadersAndInputLayout()
{
    ...
    mShaders["vizVS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "VSQuad", "vs_5_0");
    mShaders["vizPS"] = d3dUtil::CompileShader(L"Shaders\\Default.hlsl", nullptr, "PSQuad", "ps_5_0");
...
void BlendApp::BuildWavesGeometry()
{
    ...
    for (int i = 0; i < m - 1; ++i)
    {
        for (int j = 0; j < n - 1; ++j)
        {
            indices[k] = std::uint16_t(i * n + j);
            indices[k + 1] = std::uint16_t(i * n + j + 1);
            indices[k + 2] = std::uint16_t((i + 1) * n + j);

            indices[k + 3] = std::uint16_t((i + 1) * n + j);
            indices[k + 4] = std::uint16_t(i * n + j + 1);
            indices[k + 5] = std::uint16_t((i + 1) * n + j + 1);
...
{
    CD3DX12_DEPTH_STENCIL_DESC complexityCounter(D3D12_DEFAULT);
    complexityCounter.StencilEnable = true;
    complexityCounter.StencilReadMask = D3D10_DEFAULT_STENCIL_READ_MASK;
    complexityCounter.StencilWriteMask = D3D10_DEFAULT_STENCIL_WRITE_MASK;
    complexityCounter.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    complexityCounter.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
    complexityCounter.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_ALWAYS;
    complexityCounter.FrontFace.StencilPassOp = D3D12_STENCIL_OP_INCR;
    ...
    opaquePsoDesc.DepthStencilState = complexityCounter;
    ...
    constexpr D3D12_RENDER_TARGET_BLEND_DESC transparencyBlendDesc = {
        .BlendEnable = true,
        .LogicOpEnable = false,
    ...
    D3D12_DEPTH_STENCIL_DESC vizComplexity = {0};
    vizComplexity.DepthEnable = false;
    vizComplexity.StencilEnable = true;
    vizComplexity.StencilReadMask = D3D10_DEFAULT_STENCIL_READ_MASK;
    vizComplexity.StencilWriteMask = D3D10_DEFAULT_STENCIL_WRITE_MASK;
    vizComplexity.FrontFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    vizComplexity.FrontFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
    vizComplexity.FrontFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
    vizComplexity.FrontFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;
    vizComplexity.BackFace.StencilDepthFailOp = D3D12_STENCIL_OP_KEEP;
    vizComplexity.BackFace.StencilFailOp = D3D12_STENCIL_OP_KEEP;
    vizComplexity.BackFace.StencilFunc = D3D12_COMPARISON_FUNC_EQUAL;
    vizComplexity.BackFace.StencilPassOp = D3D12_STENCIL_OP_KEEP;

    D3D12_GRAPHICS_PIPELINE_STATE_DESC complexityVizPsoDesc = opaquePsoDesc;
    complexityVizPsoDesc.DepthStencilState = vizComplexity;
    complexityVizPsoDesc.VS =
    {
        reinterpret_cast<BYTE*>(mShaders["vizVS"]->GetBufferPointer()),
        mShaders["vizVS"]->GetBufferSize()
    };
    complexityVizPsoDesc.PS =
    {
        reinterpret_cast<BYTE*>(mShaders["vizPS"]->GetBufferPointer()),
        mShaders["vizPS"]->GetBufferSize()
    };
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
        &complexityVizPsoDesc,
        IID_PPV_ARGS(&mPSOs["vizPSO"])));
}
...
{
    mAllRitems.reserve(3);

    // Waves.
    RenderItem ri = {};
    ri.World = MathHelper::Identity4x4();
    XMStoreFloat4x4(&ri.TexTransform, XMMatrixScaling(5.0f, 5.0f, 1.0f));
    ri.ObjCBIndex = 0;
    ri.Mat = mMaterials["water"].get();
    ri.Geo = mGeometries["waterGeo"].get();
    ri.PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    ri.IndexCount = ri.Geo->DrawArgs["grid"].IndexCount;
    ri.StartIndexLocation = ri.Geo->DrawArgs["grid"].StartIndexLocation;
    ri.BaseVertexLocation = ri.Geo->DrawArgs["grid"].BaseVertexLocation;
    mAllRitems.push_back(ri);
    mWavesRitem = &mAllRitems.back();
    mRitemLayer[(int)RenderLayer::Transparent].push_back(mWavesRitem);
    ...
    XMStoreFloat4x4(&ri.TexTransform, XMMatrixScaling(5.0f, 5.0f, 1.0f));
    ri.ObjCBIndex = 1;
    ri.Mat = mMaterials["grass"].get();
    ri.Geo = mGeometries["landGeo"].get();
    ri.PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    ri.IndexCount = ri.Geo->DrawArgs["grid"].IndexCount;
    ri.StartIndexLocation = ri.Geo->DrawArgs["grid"].StartIndexLocation;
    ri.BaseVertexLocation = ri.Geo->DrawArgs["grid"].BaseVertexLocation;
    mAllRitems.push_back(ri);
    mRitemLayer[(int)RenderLayer::Opaque].push_back(&mAllRitems.back());

    // Box.
    ri = {};
    XMStoreFloat4x4(&ri.World, XMMatrixTranslation(3.0f, 2.0f, -9.0f));
    ri.ObjCBIndex = 2;
    ri.Mat = mMaterials["wirefence"].get();
    ri.Geo = mGeometries["boxGeo"].get();
    ri.PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
    ...
    mAllRitems.push_back(ri);
    mRitemLayer[(int)RenderLayer::AlphaTested].push_back(&mAllRitems.back());
}
