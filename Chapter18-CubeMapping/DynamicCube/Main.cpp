...
constexpr UINT CubeMapSize = 1024;
...
class DynamicCubeMapApp final : public D3DApp
{
public:
...
{
    const std::array<std::string, 4> texNames =
    {
        "bricksDiffuseMap",
        "tileDiffuseMap",
        "defaultDiffuseMap",
    ...
    const std::array<std::wstring, 4> texFilenames =
    {
    ...
    const size_t sz = texNames.size();
    mTextures.reserve(sz);
...
void DynamicCubeMapApp::BuildDescriptorHeaps()
{
    ...
    const auto& bricksTex = mTextures["bricksDiffuseMap"]->Resource;
    const auto& tileTex = mTextures["tileDiffuseMap"]->Resource;
    const auto& whiteTex = mTextures["defaultDiffuseMap"]->Resource;
    const auto& skyTex = mTextures["skyCubeMap"]->Resource;
...
void DynamicCubeMapApp::BuildSkullGeometry()
{
    ...
    constexpr XMFLOAT3 vMinf3(+MathHelper::Infinity, +MathHelper::Infinity, +MathHelper::Infinity);
    constexpr XMFLOAT3 vMaxf3(-MathHelper::Infinity, -MathHelper::Infinity, -MathHelper::Infinity);

    XMVECTOR vMin = XMLoadFloat3(&vMinf3);
    XMVECTOR vMax = XMLoadFloat3(&vMaxf3);

    std::unique_ptr vertices  = std::make_unique<Vertex[]>(vcount);
    ...
    const std::size_t numIndices = 3ull * tcount;
    std::unique_ptr indices = std::make_unique<std::int32_t[]>(numIndices);
    ...
    const UINT ibByteSize = UINT(numIndices) * sizeof(std::int32_t);
    ...
    geo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
        mCommandList.Get(), vertices.get(), vbByteSize, geo->VertexBufferUploader);

    geo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
        mCommandList.Get(), indices.get(), ibByteSize, geo->IndexBufferUploader);
    ...
    submesh.IndexCount = UINT(numIndices);
...
{
    constexpr UINT kPassCount = 7; // 1 for main frame 6 for cubemap
    for (auto& fr : mFrameResources)
    {
        fr = (std::make_unique<FrameResource>(md3dDevice.Get(),
            kPassCount, (UINT)mAllRitems.size(), (UINT)mMaterials.size()));
    }
}
...
{
    const auto viewPort = mDynamicCubeMap->Viewport();
    const auto scissorRect = mDynamicCubeMap->ScissorRect();
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        // Specify the buffers we are going to render to.
        const auto rtv = mDynamicCubeMap->Rtv(i);
...
void DynamicCubeMapApp::BuildCubeFaceCamera(float x, float y, float z)
{
    ...
    constexpr XMFLOAT3 worldUp(0.0f, 1.0f, 0.0f);
