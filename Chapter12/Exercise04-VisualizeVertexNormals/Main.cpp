...
private:
    ...
    std::array<D3D12_INPUT_ELEMENT_DESC, 3> mInputLayoutDefault = {};
    std::array<D3D12_INPUT_ELEMENT_DESC, 2> mInputLayoutNormalViz = {};
...
void BlendApp::Draw(const GameTimer& gt)
{
    ...
    // Draw normal viz.
    mCommandList->SetPipelineState(mPSOs["normalViz"].Get());
    mCommandList->IASetIndexBuffer(nullptr);
    mCommandList->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_POINTLIST);
    for (const RenderItem& ri : mAllRitems)
    {
        const D3D12_VERTEX_BUFFER_VIEW vertexView = ri.Geo->VertexBufferView();
        mCommandList->IASetVertexBuffers(0, 1, &vertexView);
        const UINT numVertices = vertexView.SizeInBytes / sizeof(Vertex);
        mCommandList->DrawInstanced(numVertices, 1, 0, 0);
    }
...
void BlendApp::BuildShadersAndInputLayout()
{
    ...
    mShaders["normalVizVS"] = d3dUtil::CompileShader(
        L"Shaders\\NormalVisualizer.hlsl", nullptr, "VS", "vs_5_0");
    mShaders["normalVizGS"] = d3dUtil::CompileShader(
        L"Shaders\\NormalVisualizer.hlsl", nullptr, "GS", "gs_5_0");
    mShaders["normalVizPS"] = d3dUtil::CompileShader(
        L"Shaders\\NormalVisualizer.hlsl", nullptr, "PS", "ps_5_0");

    mInputLayoutDefault[0] = { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
    mInputLayoutDefault[1] = { "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
    mInputLayoutDefault[2] = { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };

    mInputLayoutNormalViz[0] = mInputLayoutDefault[0];
    mInputLayoutNormalViz[1] = mInputLayoutDefault[1];
}
...
void BlendApp::BuildPSOs()
{
    ...
    //
    opaquePsoDesc.InputLayout = { mInputLayoutDefault.data(), (UINT)mInputLayoutDefault.size() };
    ...
    //
    // PSO for normal visualization
    //
    D3D12_GRAPHICS_PIPELINE_STATE_DESC normalVizDesc = opaquePsoDesc;
    normalVizDesc.VS =
    {
        reinterpret_cast<BYTE*>(mShaders["normalVizVS"]->GetBufferPointer()),
        mShaders["normalVizVS"]->GetBufferSize()
    };
    normalVizDesc.GS =
    {
        reinterpret_cast<BYTE*>(mShaders["normalVizGS"]->GetBufferPointer()),
        mShaders["normalVizGS"]->GetBufferSize()
    };
    normalVizDesc.PS =
    {
        reinterpret_cast<BYTE*>(mShaders["normalVizPS"]->GetBufferPointer()),
        mShaders["normalVizPS"]->GetBufferSize()
    };
    normalVizDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT;
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
        &normalVizDesc,
        IID_PPV_ARGS(&mPSOs["normalViz"])));
}
