//***************************************************************************************
// Chapter 06 Exercise 07: Merge pyramid and cube objects into a single buffer.
// 
...
void BoxApp::Draw(const GameTimer& gt)
{
    ...
    const D3D12_VERTEX_BUFFER_VIEW vertexBuffView = mGeometry->VertexBufferView();
    const D3D12_INDEX_BUFFER_VIEW indexBuffView = mGeometry->IndexBufferView();
    ...
    mCommandList->DrawIndexedInstanced(
        mGeometry->DrawArgs["pyramid"].IndexCount,
        1,
        mGeometry->DrawArgs["pyramid"].StartIndexLocation,
        mGeometry->DrawArgs["pyramid"].BaseVertexLocation,
        0);
...
{
    constexpr UINT kNumBoxVertices = 8;
    constexpr UINT kNumPyramidVertices = 5;
    constexpr UINT kNumVertices = kNumBoxVertices + kNumPyramidVertices;
    const std::array<Vertex, kNumVertices> vertices = {
        // Box vertices.
        ...
        Vertex(XMFLOAT3(2.0f, -1.0f, 1.0f), XMFLOAT4(Colors::Green)),
        Vertex(XMFLOAT3(4.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green)),
        Vertex(XMFLOAT3(4.0f, -1.0f, 1.0f), XMFLOAT4(Colors::Green)),
        Vertex(XMFLOAT3(3.0f, 1.0f, 0.0f), XMFLOAT4(Colors::Red)),
   };

    constexpr UINT kNumBoxIndices = 36;
    constexpr UINT kNumPyramidIndices = 18;
    constexpr UINT kNumIndices = kNumBoxIndices + kNumPyramidIndices;
    constexpr std::array<std::uint16_t, kNumIndices> indices = {
        // Box indices
        0, 1, 2,
        0, 2, 3,
        4, 6, 5,
    ...
    mGeometry->Name = "BoxPyramidMerge";
    ...
    CopyMemory(
        mGeometry->VertexBufferCPU->GetBufferPointer(), vertices.data(), vbByteSize);
    ...
    CopyMemory(
        mGeometry->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);
    ...
    SubmeshGeometry box;
    box.IndexCount = kNumBoxIndices;
    box.StartIndexLocation = 0;
    box.BaseVertexLocation = 0;
    mGeometry->DrawArgs.emplace("box", box);

    SubmeshGeometry pyramid;
    pyramid.IndexCount = kNumPyramidIndices;
    pyramid.StartIndexLocation = kNumBoxIndices;
    pyramid.BaseVertexLocation = kNumBoxVertices;
    mGeometry->DrawArgs.emplace("pyramid", pyramid);
}
...
void BoxApp::UpdateWorldViewProj()
{
