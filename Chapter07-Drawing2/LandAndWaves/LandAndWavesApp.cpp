...
void LandAndWavesApp::Draw(const GameTimer& gt)
{
	...
	const D3D12_CPU_DESCRIPTOR_HANDLE dsView = DepthStencilView();
...
void LandAndWavesApp::UpdateWaves(const GameTimer& gt)
{
	...
	if((mTimer.TotalTime() - t_base) >= 0.16f)
	{
		t_base += 0.16f;
...
{
	GeometryGenerator::MeshData grid = GeometryGenerator::CreateGrid(
		160.0f, 
		160.0f, 
		50, 
		50);
...
void LandAndWavesApp::BuildWavesGeometryBuffers()
{
	...
	for(int i = 0; i < m - 1; ++i)
	{
		for(int j = 0; j < n - 1; ++j)
		{
			indices[k] = static_cast<std::uint16_t>(i*n + j);
			indices[k + 1] = static_cast<std::uint16_t>(i*n + j + 1);
			indices[k + 2] = static_cast<std::uint16_t>((i + 1)*n + j);

			indices[k + 3] = static_cast<std::uint16_t>((i + 1)*n + j);
			indices[k + 4] = static_cast<std::uint16_t>(i*n + j + 1);
			indices[k + 5] = static_cast<std::uint16_t>((i + 1)*n + j + 1);
