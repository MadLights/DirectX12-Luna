...
class NormalMapApp final : public D3DApp
{
public:
...
void NormalMapApp::Draw(const GameTimer& gt)
{
    ...
    mCommandList->ClearRenderTargetView(
        backBuffView, Colors::LightSteelBlue, 0, nullptr);
...
void NormalMapApp::LoadTextures()
{
    ...
    const std::size_t textureCount = texNames.size();
    mTextures.reserve(textureCount);

    for (std::size_t i = 0; i < textureCount; ++i)
    {
        std::unique_ptr texMap = std::make_unique<Texture>();
        ...
        mTextures.emplace(texMap->Name, std::move(texMap));
    }
}
...
void NormalMapApp::BuildDescriptorHeaps()
{
    ...
    const std::array<ID3D12Resource* const, 6> tex2DList =
    {
        mTextures["bricksDiffuseMap"]->Resource.Get(),
        mTextures["bricksNormalMap"]->Resource.Get(),
        mTextures["tileDiffuseMap"]->Resource.Get(),
        mTextures["tileNormalMap"]->Resource.Get(),
        mTextures["defaultDiffuseMap"]->Resource.Get(),
        mTextures["defaultNormalMap"]->Resource.Get()
    };
    ...
    for (ID3D12Resource* const tex : tex2DList)
    {
...
void NormalMapApp::BuildShapeGeometry()
{
    ...
    std::unique_ptr geo = std::make_unique<MeshGeometry>();
    geo->Name = "shapeGeo";
...
void NormalMapApp::DrawRenderItems(
