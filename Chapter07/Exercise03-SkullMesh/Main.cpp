//***************************************************************************************
// Chapter 07 Exercise 03: Load & display the Skull mesh model from file (without normals)
//
...
void ShapesApp::Draw(const GameTimer& gt)
{
    ...
    const int passCbvIndex = mPassCbvOffset + mCurrFrameResourceIndex;
...
{    
    ...
    if (!fileIn)
    {
        throw std::runtime_error("Failed to open file: Models/skull.txt");
    }
    ...
    int numVertices;
    fileIn >> ignore >> numVertices;
    ignore.clear();

    // Vertex = (pos, color)
    std::unique_ptr vertices = std::make_unique<std::pair<XMFLOAT3, XMFLOAT4>[]>(numVertices);

    int numTriangles;
    fileIn >> ignore >> numTriangles;
    ignore.clear();

    const int numIndices = numTriangles * 3;
    std::unique_ptr indices = std::make_unique<std::uint16_t[]>(numIndices);

    constexpr long long kMaxStreamSize = std::numeric_limits<std::streamsize>::max();

    // Skip three lines.
    fileIn.ignore(kMaxStreamSize, '\n');
    fileIn.ignore(kMaxStreamSize, '\n');
    fileIn.ignore(kMaxStreamSize, '\n');

    constexpr XMFLOAT4 kRed(0.8f, 0.0f, 0.0f, 1.0f);
    for (int i = 0; i < numVertices; ++i)
    {
        XMFLOAT3& pos = vertices[i].first;
        XMFLOAT4& color = vertices[i].second;
    ...
    std::unique_ptr skullGeo = std::make_unique<MeshGeometry>();
    skullGeo->Name = "skull";
    
    const UINT vbByteSize = UINT(sizeof(vertices[0]) * numVertices);
    ThrowIfFailed(D3DCreateBlob(vbByteSize, &skullGeo->VertexBufferCPU));
    CopyMemory(skullGeo->VertexBufferCPU->GetBufferPointer(), vertices.get(), vbByteSize);
    vertices.reset();
    skullGeo->VertexBufferByteSize = vbByteSize;
    skullGeo->VertexByteStride = sizeof(vertices[0]);

    const UINT ibByteSize = UINT(sizeof(std::uint16_t) * numIndices);
    ThrowIfFailed(D3DCreateBlob(ibByteSize, &skullGeo->IndexBufferCPU));
    CopyMemory(skullGeo->IndexBufferCPU->GetBufferPointer(), indices.get(), ibByteSize);
    indices.reset();
    skullGeo->IndexBufferByteSize = ibByteSize;
    skullGeo->IndexFormat = DXGI_FORMAT_R16_UINT;
    
    skullGeo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(
        md3dDevice.Get(),
        mCommandList.Get(),
        skullGeo->VertexBufferCPU->GetBufferPointer(),
        vbByteSize,
        skullGeo->VertexBufferUploader);
    skullGeo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(
        md3dDevice.Get(),
        mCommandList.Get(),
        skullGeo->IndexBufferCPU->GetBufferPointer(),
        ibByteSize,
        skullGeo->IndexBufferUploader);

    const SubmeshGeometry skull = {
        .IndexCount = UINT(numIndices),
        .StartIndexLocation = 0,
        .BaseVertexLocation = 0,
    };
    skullGeo->DrawArgs.emplace("skull", skull);
    mGeometries.emplace(skullGeo->Name, std::move(skullGeo));
}
...
void ShapesApp::BuildRenderItems()
{
    ...
    XMStoreFloat4x4(&skullRitem->World, XMMatrixIdentity());
    ...
    const std::vector<RenderItem*>& ritems)
{
    ...
    {
        vbView = ri->Geo->VertexBufferView();
        ibView = ri->Geo->IndexBufferView();
