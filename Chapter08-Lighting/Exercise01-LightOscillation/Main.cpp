//***************************************************************************************
// Chapter 08 Exercise 01: Pulse a red light based on time.
// 
...
static float GetHillsHeight(const float x, const float z)
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

static XMFLOAT3 GetHillsNormal(const float x, const float z)
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n(
...
void LitWavesApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	mSunPhi = std::clamp(mSunPhi, 0.1f, XM_PIDIV2);
}
...
void LitWavesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	XMStoreFloat3(&mMainPassConstants.Lights[0].Direction, lightDir);

	const float red = (1.0f + std::sinf(gt.TotalTime() * XM_2PI)) * 0.5f;
	mMainPassConstants.Lights[0].Strength = { red, 0.0f, 0.0f };
