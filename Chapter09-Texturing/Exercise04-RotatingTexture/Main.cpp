...
class CrateApp final : public D3DApp
{
public:
...
void CrateApp::AnimateTextures(const GameTimer& gt)
{
    const float t = gt.TotalTime();
    XMMATRIX texTransform;
    for (auto& e : mAllRitems)
    {
        texTransform = XMMatrixTranslation(std::cosf(t), std::sinf(t), 0.0f);
...
void CrateApp::LoadTextures()
{
    ...
    ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
        ...
        flareTex->Resource,
        flareTex->UploadHeap));
    mTextures.emplace(flareTex->Name, std::move(flareTex));
    ...
    mTextures.emplace(flareAlphaTex->Name, std::move(flareAlphaTex));
}
...
void CrateApp::BuildDescriptorHeaps()
{
    ...
    const auto& flareTex = mTextures["flareTex"]->Resource;
    ...
    const auto& flareAlphaTex = mTextures["flareAlphaTex"]->Resource;
...
void CrateApp::BuildShadersAndInputLayout()
{
    mShaders["standardVS"] = d3dUtil::CompileShader(
        L"Shaders\\RotatingTexture.hlsl", nullptr, "VS", "vs_5_0");
    mShaders["opaquePS"] = d3dUtil::CompileShader(
        L"Shaders\\RotatingTexture.hlsl", nullptr, "PS", "ps_5_0");
