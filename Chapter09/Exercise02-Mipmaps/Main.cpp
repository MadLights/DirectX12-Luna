...
{
	auto mipMapTex = std::make_unique<Texture>();
	mipMapTex->Name = "mipMapTex";
	mipMapTex->Filename = L"../../Textures/mipmaps.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(
		md3dDevice.Get(),
		mCommandList.Get(), 
		mipMapTex->Filename.c_str(),
		mipMapTex->Resource, 
		mipMapTex->UploadHeap));
 
	mTextures[mipMapTex->Name] = std::move(mipMapTex);
}
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	const auto& mipMapTex = mTextures["mipMapTex"]->Resource;
 
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc.Format = mipMapTex->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = mipMapTex->GetDesc().MipLevels;
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

	md3dDevice->CreateShaderResourceView(mipMapTex.Get(), &srvDesc, hDescriptor);
}
