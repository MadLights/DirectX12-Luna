//***************************************************************************************
// Chapter 14 Exercise 02: Tesselate an iscoahedron into a sphere.
// 
...
class BasicTessellationApp final : public D3DApp
{
public:
...
void BasicTessellationApp::BuildIscoahedronGeometry()
{
    constexpr float X = 0.525731f;
    constexpr float Z = 0.850651f;

    constexpr std::array<XMFLOAT3, 12> vertices =
    {
    ...
    constexpr std::array<std::uint16_t, 60> indices =
    {
    ...
    constexpr UINT vbByteSize = (UINT)vertices.size() * sizeof(vertices[0]);
    constexpr UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);

    auto geo = std::make_unique<MeshGeometry>();
    geo->Name = "icoPatchGeo";
    ...
    const SubmeshGeometry quadSubmesh = {
        .IndexCount = UINT(indices.size()),
        .StartIndexLocation = 0,
        .BaseVertexLocation = 0
    };

    geo->DrawArgs["icoPatch"] = quadSubmesh;
...
void BasicTessellationApp::BuildPSOs()
{
    ...
    //
    opaquePsoDesc.InputLayout = { &mInputLayout, 1 };
...
{
    auto icoaPathRitem = std::make_unique<RenderItem>();
    icoaPathRitem->World = MathHelper::Identity4x4();
    icoaPathRitem->TexTransform = MathHelper::Identity4x4();
    icoaPathRitem->ObjCBIndex = 0;
    icoaPathRitem->Mat = mMaterials["whiteMat"].get();
    icoaPathRitem->Geo = mGeometries["icoPatchGeo"].get();
    icoaPathRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
    icoaPathRitem->IndexCount = icoaPathRitem->Geo->DrawArgs["icoPatch"].IndexCount;
    icoaPathRitem->StartIndexLocation = icoaPathRitem->Geo->DrawArgs["icoPatch"].StartIndexLocation;
    icoaPathRitem->BaseVertexLocation = icoaPathRitem->Geo->DrawArgs["icoPatch"].BaseVertexLocation;
    mRitemLayer[(int)RenderLayer::Opaque].push_back(icoaPathRitem.get());

    mAllRitems.push_back(std::move(icoaPathRitem));
}
...
void BasicTessellationApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    constexpr UINT matCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(MaterialConstants));
